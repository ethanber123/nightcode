    ...
    async ({ mode, model }: Omit<SubmitParams, "userText">) => {
      await runStream({
        ...
        request: async (controller) => {
          // apiClient.chat may have typed index signatures; cast to any to avoid
          // narrow typing issues at call sites
          return (apiClient.chat as any)[":sessionId"].resume.$post(
            { param: { sessionId } },
  ...
  const submit = useCallback(
    async ({ userText, mode, model }: SubmitParams) => {
      ...
      await runStream({
        ...
        model,
        request: async (controller) => {
          return (apiClient.chat as any)[":sessionId"].$post(
            {
              param: { sessionId },
